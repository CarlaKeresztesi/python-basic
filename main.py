print('I am learning Python!')

#   Data Types

# string = text contained within quotes - either "" or ''(e.g. "This is a string" or 'This is a string')

# number can be
    # integers = any non-floating point numbers e.g. 100, 25, 1
    # floats or floating point numbers = the one containing a decimal point e.g. 9.99
    # to check the type of a number:
print(type(9.99))
print(type(100))
    # if we add a float and an integer together, the output will be a float

# boolean
    # true
print(True)
    # false
print(False)